...
describe("student, goal, and progress validation", () => {
  it("validates student date/status payloads", () => {
    ...
    expect(studentSchema.safeParse({ organizationId, firstName: "", lastName: "Learner" }).success).toBe(false);
  });

  it("validates IEP cycle date ordering", () => {
    expect(iepCycleSchema.safeParse({ organizationId, studentId, label: "Annual", startDate: "2026-01-10", endDate: "2026-01-01" }).success).toBe(false);
  });

  it("validates IEP goals with target dates", () => {
    expect(iepGoalSchema.safeParse({
      organizationId,
      studentId,
      iepCycleId,
      goalArea: "Reading",
  ...
  it("validates percentage and reading fluency progress", () => {
    expect(percentageProgressSchema.parse({
      organizationId,
      studentId,
      goalId,
    ...
    expect(readingFluencyProgressSchema.safeParse({
      organizationId,
      studentId,
      goalId,
      sessionDate: "2026-01-01",
