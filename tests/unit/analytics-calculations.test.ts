...
const points: ObservationPoint[] = [
  { date: "2026-01-01", value: 20, measurementType: "percentage", status: "finalized", phaseId: "baseline", setting: "classroom", promptLevel: "verbal" },
  { date: "2026-01-01", value: 30, measurementType: "percentage", status: "finalized", phaseId: "baseline", setting: "classroom", promptLevel: "verbal" },
  { date: "2026-01-08", value: 40, measurementType: "percentage", status: "draft", phaseId: "intervention", setting: "therapy", promptLevel: "model" },
  { date: "2026-01-15", value: 50, measurementType: "percentage", status: "corrected", phaseId: "intervention", setting: "therapy", promptLevel: "model" },
  { date: "2026-01-22", value: null, measurementType: "percentage", status: "finalized", phaseId: "maintenance", setting: "home", promptLevel: "none" },
  { date: "2026-02-01", value: 60, measurementType: "percentage", status: "finalized", phaseId: "generalization", setting: "community", promptLevel: "none" },
];
...
describe("analytics calculations", () => {
  ...
  it("summarizes task analysis steps", () => {
    expect(summarizeTaskAnalysis(["independent", "prompted", "incorrect", "not_attempted"])).toEqual({
      independent: 1,
      prompted: 1,
      incorrect: 1,
      notAttempted: 1,
  ...
  it("calculates trend and ROI with higher-is-better direction", () => {
    expect(linearTrend([{ x: 1, y: 2 }, { x: 2, y: 4 }, { x: 3, y: 6 }])).toEqual({ slope: 2, intercept: 0 });
    expect(rateOfImprovement(points, true)).toMatchObject({ value: 14, direction: "improving", sufficiency: { status: "ok" } });
  });
  ...
  it("returns unavailable trend for zero or one finalized point", () => {
    ...
    expect(rateOfImprovement([{ date: "2026-01-01", value: 1, measurementType: "rate", status: "finalized" }], true).sufficiency.status).toBe("unavailable");
  });

  it("calculates aim lines and handles missing or invalid dates", () => {
    expect(aimLine({ baseline: 10, target: 30, startDate: "2026-01-01", targetDate: "2026-01-11", onDate: "2026-01-06" })).toEqual({ value: 20, sufficiency: { status: "ok" } });
    expect(aimLine({ baseline: 10, target: 30, startDate: "", targetDate: "2026-01-11", onDate: "2026-01-06" }).sufficiency.status).toBe("unavailable");
    expect(aimLine({ baseline: 10, target: 30, startDate: "2026-01-11", targetDate: "2026-01-01", onDate: "2026-01-06" }).sufficiency.status).toBe("unavailable");
  });
  ...
  it("rejects incompatible units and excludes draft-only data", () => {
    ...
    expect(describeSeries([{ date: "2026-01-01", value: 1, measurementType: "rate", status: "draft" }]).sufficiency.status).toBe("unavailable");
  });
