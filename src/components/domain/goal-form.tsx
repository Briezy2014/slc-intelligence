    ...
    <div className="space-y-6">
      ...
      <form action={submitAction(saveGoalAction)} className="space-y-4">
        ...
        <Alert title="Learning progressions" tone="info">
          Choose a grade level and course subject to load progression goals (including functional
          math and ASL communication). After mastery, generate the next progression step.
        </Alert>
              ...
              <div className="flex flex-wrap gap-2">
                {nextProgressions.map((entry) => (
                  <Button
                    ...
                    type="button"
                    variant="secondary"
                    onClick={() => applyProgression(entry.id)}
                  >
        ...
        <FormField id="status" label="Goal status">
          <Select
            ...
            onChange={(event) =>
              setStatus(
                event.target.value as "active" | "inactive" | "archived" | "mastered_review",
              )
            }
          >
