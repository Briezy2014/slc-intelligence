...
export function isAiAssistEnabled(): boolean {
  const flag = (process.env.AI_ASSIST_ENABLED ?? process.env.NEXT_PUBLIC_AI_ASSIST_ENABLED ?? "true")
    .trim()
    .toLowerCase();
...
export function getAiProviderConfig():
  | { configured: true; apiKey: string; baseUrl: string; model: string }
  | { configured: false } {
  const apiKey = (process.env.AI_API_KEY ?? "").trim();
  if (!apiKey) return { configured: false };

  const baseUrl = (process.env.AI_API_BASE_URL ?? "https://api.openai.com/v1").trim().replace(/\/$/, "");
