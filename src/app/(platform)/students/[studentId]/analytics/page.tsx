...
export default async function StudentAnalyticsPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  const points = progressState.configured && !progressState.error
    ? toObservationPoints(progressState.data.sessions)
    : [];

  return (
    <main id="main-content">
      <Breadcrumbs items={[{ href: "/", label: "Home" }, { href: "/students", label: "Students" }, { label: "Analytics" }]} />
      <PageHeader title="Student analytics" description="Tables-first summaries for authorized progress data." />
