...
export default async function StudentIepPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  return (
    <main id="main-content">
      ...
      <PageHeader title="Student IEP" description="IEP cycles for the selected authorized student." />
      {!studentState.configured || !goalsState.configured ? <ConfigurationState /> : studentState.error || goalsState.error ? (
        ...
        <div className="space-y-6">
          <TableShell caption="IEP cycles" headers={["Label", "Start", "End", "Review", "Status"]} rows={goalsState.data.cycles.filter((cycle) => cycle.student_id === studentId).map((cycle) => [cycle.label, cycle.start_date, cycle.end_date ?? "", cycle.review_date ?? "", cycle.status])} />
          {goalsState.data.canManage ? (
            <Card>
              ...
              <div className="mt-4"><IepCycleForm organizationId={goalsState.data.organizationId} studentId={studentId} /></div>
            </Card>
          ) : null}
        </div>
