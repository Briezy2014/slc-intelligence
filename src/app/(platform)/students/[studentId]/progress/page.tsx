...
export default async function StudentProgressPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  const [studentState, progressState] = await Promise.all([getStudent(studentId), listProgress({ studentId })]);

  return (
    <main id="main-content">
      <Breadcrumbs items={[{ href: "/", label: "Home" }, { href: "/students", label: "Students" }, { label: "Progress" }]} />
      <PageHeader
      ...
      />
      {!studentState.configured || !progressState.configured ? <ConfigurationState /> : studentState.error || progressState.error ? (
        ...
        <TableShell
          ...
          rows={progressState.data.sessions.map((session) => [session.session_date, session.goal_id, session.measurement_type.replaceAll("_", " "), session.value === null ? "No numeric value" : String(session.value), session.status])}
        />
