...
export default async function GoalAnalyticsPage({ params }: { params: Promise<{ goalId: string }> }) {
  ...
  return (
    <main id="main-content">
      <Breadcrumbs items={[{ href: "/", label: "Home" }, { href: "/goals", label: "Goals" }, { label: "Goal analytics" }]} />
      <PageHeader title="Goal analytics" description="Accessible trend and descriptive summaries for the selected goal." />
      {!goalState.configured || !progressState.configured ? <ConfigurationState /> : goalState.error || progressState.error ? (
        ...
        <GoalProgressChart title={`${goal.goal_area} progress`} points={points} higherIsBetter={goal.target_direction === "increase"} />
