...
export default async function ClassroomDetailPage({ params }: { params: Promise<{ classroomId: string }> }) {
  ...
  return (
    <main id="main-content">
      <Breadcrumbs items={[{ href: "/", label: "Home" }, { href: "/classrooms", label: "Classrooms" }, { label: "Classroom detail" }]} />
      <PageHeader
        title={classroom?.name ?? "Classroom detail"}
        description="Classroom metadata and scope."
        actions={classroom && state.data.canManage ? (
          <Link href={`/classrooms/${classroom.id}/edit`} className="bg-accent text-accent-foreground rounded-[var(--radius-md)] px-4 py-2 text-sm font-semibold">Edit classroom</Link>
        ) : null}
      />
      {!state.configured ? <ConfigurationState /> : state.error ? <SafeErrorState message={state.error} /> : classroom ? (
